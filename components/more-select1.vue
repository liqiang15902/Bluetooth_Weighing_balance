	...
	<view class="city-box">
		...
		<view class="choosed-box" v-if="chooseCityList.length > 0">
			<view class="city-single" v-for="(item,index) in chooseCityList" :key="index">
				<text :style="'color:' + colorValue" style="font-size: 20rpx;">{{item.servgrpname}}</text>
			...
			<view class="level-one-data">
				<scroll-view scroll-y="true" :style="'height:' + (900-viewHeight) + 'upx'" @scroll="scroll"
					...
					<view class="county-single" v-for="(zItem,zIndex) in cityData" :key="zIndex"
						...
						<view v-if="cityIdArr.includes(zItem.servgrpcode)" class="city-name" :style="'color:' + colorValue">
							{{zItem.servgrpname}}</view>
						<view v-else class="city-name">{{zItem.servgrpname}}</view>
						<view v-if="cityIdArr.includes(zItem.servgrpcode)" class="selected-box" :style="'background:' + colorValue + ';border:1px solid ' +colorValue">
